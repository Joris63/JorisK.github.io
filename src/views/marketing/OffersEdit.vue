<script setup lang="ts">
  ...
  interface Group   { id: number; naam: string; expanded: boolean; children: Group[]; products: Product[]; }
  ...
  const groups = ref<Group[]>([
    {
      id: 1, naam: '2026 Q1: GROHE COLORS BE', expanded: true, products: [],
      children: [
        {
          id: 2, naam: 'Douche & Bad', expanded: true, products: [],
          children: [
            { id: 3, naam: 'Douchestellen',     expanded: false, children: [], products: P_DOUCHESTELLEN     },
            { id: 4, naam: 'Thermostaatkranen', expanded: false, children: [], products: P_THERMOSTAATKRANEN },
  ...
  function nextId(): number {
    ...
    function traverse(ns: Group[]) {
      for (const g of ns) { if (g.id > max) max = g.id; traverse(g.children); }
    }
...
function findPath(targetId: number, nodes: Group[], path: Group[] = []): Group[] | null {
  ...
  function handleGroupSave(data: { isEdit: boolean; editGroupId: number | null; parentGroupId: number | null; naam: string }) {
    if (data.isEdit) {
      ...
      if (!parent) return;
      const newId = nextId();
      parent.children.push({ id: newId, naam: data.naam, expanded: false, children: [], products: [] });
      parent.expanded = true;
      selectNode(newId);
    } else {
      const newId = nextId();
      groups.value.push({ id: newId, naam: data.naam, expanded: false, children: [], products: [] });
      selectNode(newId);
    }
  }
  ...
  function goBack() { router.push({ name: 'OffersOverview' }); }

  // ── Pending midnight changes ───────────────────────────────────────
  interface PendingProductChange { code: string; merk: string; action: 'add' | 'remove'; }
  interface PendingOptionChange  { label: string; icon: string; action: 'add' | 'remove'; }
  interface PendingGroupChanges  { groupAction?: 'add' | 'remove'; options: PendingOptionChange[]; products: PendingProductChange[]; }
  ...
  const selectedGroupPending = computed((): PendingGroupChanges | null => PENDING_CHANGES[selectedNodeId.value] ?? null);
  const pendingAddCount      = computed(() => {
    const p = selectedGroupPending.value;
    if (!p || p.groupAction) return 0;
    return p.options.filter(c => c.action === 'add').length + p.products.filter(c => c.action === 'add').length;
  });
  const pendingRemoveCount   = computed(() => {
    const p = selectedGroupPending.value;
    if (!p || p.groupAction) return 0;
    return p.options.filter(c => c.action === 'remove').length + p.products.filter(c => c.action === 'remove').length;
  });

  function groupHasPending(id: number) { return id in PENDING_CHANGES; }
  function groupHasPendingRemove(id: number) {
    const p = PENDING_CHANGES[id];
    return p ? (p.groupAction === 'remove' || p.options.some(c => c.action === 'remove') || p.products.some(c => c.action === 'remove')) : false;
  }
  function groupScheduledAction(id: number) { return PENDING_CHANGES[id]?.groupAction ?? null; }

  function deleteGroup(id: number) {
    function removeFrom(nodes: Group[]): boolean {
      const idx = nodes.findIndex(g => g.id === id);
      if (idx !== -1) { nodes.splice(idx, 1); return true; }
      for (const g of nodes) { if (removeFrom(g.children)) return true; }
      return false;
    }
  ...
  watch(selectedNodeId, () => { wijzigingenOpen.value = false; });
</script>
  ...
  <div class="page">
    ...
    <div class="page-header">
      ...
      <div class="flex items-center gap-2 shrink-0">
        ...
        <Popover ref="moreMenuRef">
          <div class="ctx-menu">
            <button class="ctx-item" @click="showDuplicateDialog = true; moreMenuRef?.hide()"><i class="pi pi-copy" />Kopieer actie</button>
            <div class="ctx-sep" />
    ...
    <Transition name="fade">
      <div v-if="isVerlopen" class="verlopen-banner">
        ...
        <button class="verlopen-copy-btn" @click="showDuplicateDialog = true">Maak een kopie</button>
      </div>
    </Transition>
    ...
    <div class="groepen-body">
      ...
      <div class="groups-panel">
        ...
        <div class="groups-tree">
          ...
          >
            <button v-if="node.children.length" class="expand-btn" @click.stop="toggleGroup(node.id)">
            ...
            <div class="row-actions">
              <button v-if="!isVerlopen" class="row-action-btn" title="Subgroep toevoegen" @click.stop="openCreateGroup(node.id)">
                <i class="pi pi-plus text-xs" />
              </button>
              <button class="row-action-btn" title="Bewerken" @click.stop="openEditGroup(node.id)">
                <i class="pi pi-pencil text-xs" />
              </button>
              <button v-if="!isVerlopen" class="row-action-btn row-action-btn--danger" title="Groep verwijderen" @click.stop="deleteGroup(node.id)">
        ...
        <template v-if="selectedNode">
          ...
          <div class="products-panel__header">
            ...
            <div class="header-group-actions">
              <Button icon="pi pi-pencil" severity="secondary" variant="text" size="small" rounded title="Groep bewerken" @click="openEditGroup(selectedNodeId)" />
              <Button v-if="!isVerlopen" icon="pi pi-trash" severity="danger" variant="text" size="small" rounded title="Groep verwijderen" @click="deleteGroup(selectedNodeId)" />
            </div>
          </div>
            ...
            <div v-if="selectedGroupPending" class="wijz-strip">
              <button class="wijz-hdr" @click="wijzigingenOpen = !wijzigingenOpen">
                <div class="wijz-hdr__left">
                  ...
                  <template v-if="selectedGroupPending!.groupAction">
                    <span class="wijz-badge" :class="selectedGroupPending!.groupAction === 'remove' ? 'wijz-badge--remove' : 'wijz-badge--add'">
                      Groep {{ selectedGroupPending!.groupAction === 'remove' ? 'verwijderd' : 'toegevoegd' }}
                    </span>
                  </template>
                  <template v-else>
              ...
              <Transition name="collapse">
                <div v-if="wijzigingenOpen" class="wijz-list">
                  ...
                  <template v-if="selectedGroupPending!.groupAction">
                    <div class="wijz-group-action" :class="`wijz-group-action--${selectedGroupPending!.groupAction}`">
                      <i :class="`pi ${selectedGroupPending!.groupAction === 'remove' ? 'pi-trash' : 'pi-plus-circle'}`" />
                      <span>Deze groep wordt vannacht {{ selectedGroupPending!.groupAction === 'remove' ? 'verwijderd' : 'toegevoegd' }}</span>
                    </div>
                  </template>
                  ...
                  <template v-if="selectedGroupPending!.products.length">
                    <div v-if="selectedGroupPending!.options.length" class="wijz-section-label">Producten</div>
                    <div
          ...
          <div class="products-toolbar">
            ...
            <template v-if="!isVerlopen">
              <Button label="Toevoegen" icon="pi pi-plus" size="small" @click="showAddProducts = true" />
              <Button
                icon="pi pi-trash"
                label="Verwijderen"
                severity="danger"
...
<style scoped>
  ...
  /* ── Groepen body ────────────────────────────────────────────── */
  .groepen-body { display: flex; gap: 0; flex: 1; min-height: 0; overflow: hidden; }
  ...
  .group-row--active .item-count { background: var(--p-primary-100); color: var(--p-primary-600); }
  ...
  .products-pager {
    border-top: 1px solid var(--p-surface-100); background: transparent;
    flex-shrink: 0; padding: 0.125rem 0.5rem;
  }
  ...
  .products-pager :deep(.p-paginator-last) { min-width: 2rem; height: 2rem; font-size: 0.8125rem; }
  ...
  .pending-clock         { font-size: 0.6rem; flex-shrink: 0; margin-right: -0.125rem; }
  .pending-clock--add    { color: var(--p-amber-400); }
  .pending-clock--mixed  { color: var(--p-red-400); }

  /* ── Group scheduled states ──────────────────────────────────── */
  .group-row--removing .group-naam { text-decoration: line-through; color: var(--p-red-400); }
  .group-row--adding   .group-naam { color: var(--p-green-600); font-style: italic; }
  ...
  .row-action-btn--danger:hover { background: var(--p-red-50) !important; color: var(--p-red-500) !important; }

  /* ── Wijzigingen vannacht strip ───────────────────────────────── */
  .wijz-strip  { flex-shrink: 0; border-bottom: 1px solid var(--p-gray-100); }
  .wijz-hdr {
    display: flex; align-items: center; justify-content: space-between;
    width: 100%; padding: 0.375rem 1rem;
  ...
  .wijz-badge {
    ...
    font-size: 0.73rem; font-weight: 700; line-height: 1.4;
  }
  ...
  .wijz-list { display: flex; flex-direction: column; padding: 0 1rem 0.625rem; gap: 0.2rem; }
  ...
  .wijz-section-label { font-size: 0.6875rem; font-weight: 700; text-transform: uppercase; letter-spacing: 0.05em; color: var(--p-gray-400); padding: 0.25rem 0 0.125rem; }
