...
@reference "@/assets/style.css";
  .wijz-strip { @apply shrink-0 border-b border-(--p-gray-100); }
  ...
  .wijz-hdr__label { @apply text-xs font-medium text-(--p-gray-500); }
  .wijz-clock { @apply text-xs text-(--p-gray-400); }
  .wijz-chevron { @apply text-(--p-gray-400); font-size: 0.6rem; }
  .wijz-badge { @apply text-xs inline-flex items-center rounded-full font-bold leading-[1.4]; padding: 0.05rem 0.4rem; }
  .wijz-badge--add { @apply bg-(--p-green-100) text-(--p-green-700); }
  .wijz-badge--remove { @apply bg-(--p-red-100) text-(--p-red-600); }
  .wijz-list { @apply flex flex-col overflow-y-auto px-4 pb-2.5; gap: 0.2rem; max-height: 16rem; }
  .wijz-show-more { @apply self-start cursor-pointer text-xs text-(--p-primary-500) font-medium; margin-top: 0.1rem; padding: 0; background: none; border: none; }
  .wijz-show-more:hover { text-decoration: underline; }
  .wijz-group-action { @apply text-base flex items-center rounded-md font-medium gap-2 py-1.5 px-2; }
  .wijz-group-action--remove { @apply bg-(--p-red-50) text-(--p-red-600); }
  .wijz-group-action--add { @apply bg-(--p-green-50) text-(--p-green-700); }
  .wijz-group-action .pi { font-size: 0.75rem; }
  .wijz-section-label { @apply text-xs flex items-center font-bold uppercase text-(--p-gray-400) gap-1.5 pt-1 pb-0.5; letter-spacing: 0.05em; }
  .wijz-prod-counts { @apply flex gap-1; }
  .wijz-prod-count--add { @apply text-(--p-green-600) font-bold; }
  .wijz-prod-count--remove { @apply text-(--p-red-500) font-bold; }
  .wijz-separator { @apply h-px bg-(--p-gray-100) my-1.5; }
  ...
  .wijz-opt-chip--add { @apply bg-(--p-green-50) text-(--p-green-700) border-(--p-green-200); }
  .wijz-opt-chip--remove { @apply bg-(--p-red-50) text-(--p-red-600) border-(--p-red-200); }
  .wijz-opt-icon { font-size: 0.65rem; }
  .wijz-opt-action { @apply font-normal opacity-70; }
  .wijz-item { @apply text-base flex items-center gap-2; }
  .wijz-sign { @apply font-bold text-center shrink-0; width: 0.75rem; }
  .wijz-item--add .wijz-sign { @apply text-(--p-green-600); }
  .wijz-item--remove .wijz-sign { @apply text-(--p-red-500); }
  .wijz-code { @apply text-xs font-medium text-(--p-surface-700); font-family: monospace; }
  .wijz-merk { @apply text-(--p-gray-400) text-xs whitespace-nowrap overflow-hidden; text-overflow: ellipsis; }
  /* collapse transition — defined globally in cards.css */
  .pdlg-body { @apply flex flex-col gap-3; }
  .pdlg-summary { @apply text-base flex items-center font-medium gap-2; }
  .pdlg-sum--add { @apply text-(--p-green-600); }
  .pdlg-sum--remove { @apply text-(--p-red-500); }
  .pdlg-sum-sep { @apply text-(--p-gray-300); }
  .pdlg-search { @apply w-full; }
  .pdlg-list { @apply flex flex-col overflow-y-auto; gap: 0.15rem; max-height: 22rem; }
  .pdlg-item { @apply text-base flex items-center gap-2; padding: 0.1rem 0; }
  .pdlg-sign { @apply font-bold text-center shrink-0; width: 0.75rem; }
  .pdlg-item--add .pdlg-sign { @apply text-(--p-green-600); }
  .pdlg-item--remove .pdlg-sign { @apply text-(--p-red-500); }
  .pdlg-code { @apply text-xs font-medium text-(--p-surface-700) shrink-0; font-family: monospace; }
  .pdlg-merk { @apply text-(--p-gray-400) text-xs whitespace-nowrap overflow-hidden; text-overflow: ellipsis; }
  .pdlg-empty { @apply text-base text-(--p-gray-400) text-center py-4; }
</style>
