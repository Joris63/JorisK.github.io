  ...
  <template v-else>
    ...
    <div class="products-table-wrap">
      <DataTable
        ...
        :pt="{
          columnHeaderCell: { class: 'text-xs font-semibold uppercase text-(--sw-text-muted) bg-transparent border-b border-(--sw-border-md) py-2 px-3 tracking-[0.05em]' },
          bodyRow: { class: 'group transition-[background] duration-[var(--sw-duration-fast)]' },
          bodyCell: { class: 'border-b border-(--sw-border) py-2.5 px-3 group-hover:bg-(--sw-bg-subtle) [tr:last-child_&]:border-b-0' },
        }"
      >
        <Column selection-mode="multiple" style="width: 2.5rem; padding-left: 0.75rem" />
        <Column field="code" header="Productcode">
          <template #body="{ data }"><span class="font-medium text-(--sw-text-strong) tabular-nums">{{ data.code }}</span></template>
        </Column>
        ...
        <Column field="brand" header="Merk / Serie">
          <template #body="{ data }"><span class="text-(--sw-text-label) text-base">{{ data.brand }}</span></template>
        </Column>
      </DataTable>
    </div>
  </template>
...
@reference "@/assets/style.css";
  .products-toolbar { @apply flex items-center border-b border-(--sw-border) shrink-0 gap-2 py-1.5 px-3; }
  .products-pager { @apply border-b border-(--sw-border) bg-transparent shrink-0 py-0.5 px-2; }
  ...
  .products-pager :deep(.p-paginator-last) { @apply text-base; min-width: 2rem; height: 2rem; }
  .products-empty { @apply flex-1 flex flex-col items-center justify-center text-center gap-2.5 py-12 px-8; }
  .products-empty__icon { @apply flex items-center justify-center bg-(--sw-bg-muted) text-(--sw-text-muted) rounded-full mb-1 w-14 h-14; font-size: 1.375rem; }
  .products-empty__title { @apply text-base font-semibold text-(--sw-text-sub); }
  .products-empty__sub { @apply text-base text-(--sw-text-muted); }
  .products-empty__reset { @apply text-base text-primary-600 cursor-pointer; background: none; border: none; padding: 0; transition: color var(--sw-duration-fast) var(--sw-ease); }
  .products-empty__reset:hover { @apply text-primary-800; text-decoration: underline; }
  .products-table-wrap { @apply flex-1 overflow-y-auto flex flex-col; }
  .site-tag { @apply inline-flex items-center rounded-md text-xs font-medium bg-(--sw-bg-muted) text-(--sw-text-label) whitespace-nowrap py-0.5 px-2; }
  .selection-bar { @apply fixed z-50 bottom-6; left: 50%; transform: translateX(-50%); pointer-events: none; }
  .selection-bar__inner { @apply flex items-center bg-(--sw-text) text-white whitespace-nowrap gap-4 py-2 pr-2.5 pl-5; border-radius: 2rem; box-shadow: 0 4px 24px rgba(0,0,0,0.22); pointer-events: all; }
  .selection-bar__count { @apply text-base font-medium; }
  .selection-bar__actions { @apply flex items-center gap-2; }
  .selection-bar__clear { @apply text-base cursor-pointer; color: rgba(255,255,255,0.55); background: none; border: none; padding: 0; transition: color var(--sw-duration-fast) var(--sw-ease); }
  .selection-bar__clear:hover { color: white; }
  .ctx-menu { @apply flex flex-col; min-width: 10rem; }
  .ctx-item { @apply flex items-center w-full text-left text-base text-(--sw-text-sub) cursor-pointer rounded gap-2 py-2 px-3; background: none; border: none; transition: background var(--sw-duration-fast) var(--sw-ease); }
  .ctx-item:hover { @apply bg-(--sw-bg-subtle); }
  .ctx-item--danger { @apply text-red-600; }
  .ctx-item--danger:hover { @apply bg-red-50; }
  .ctx-sep { @apply h-px bg-[var(--p-gray-100)] my-1; }
  .slide-up-enter-active { transition: all 0.22s cubic-bezier(0.34,1.56,0.64,1); }
  .slide-up-leave-active { transition: all var(--sw-duration-base) var(--sw-ease); }
